...
declare -A CMDS_HELP=(
 ...
 [run]="Run dgt commands: ./control.sh c1_1 run <family> <cmd>"
 [ps]="List containers: ./control.sh c1_1 ps [-q/--services]"

)
if [ ! -v FCOMPOSE ]; then
FCOMPOSE="docker/docker-compose-netCN-dgt-val-pbft.yaml"
fi
...
DGT_NOTA_PARAMS=(PEER PNM CRYPTO_BACK USER_NOTARY LADDR NREST SEAL_ADDR REST_API HTTPS_MODE ACCESS_TOKEN DGT_TOKEN)
fi
...
function doTokenDgt {
    ...
    doDockerCmd $container_name "dgt token get -u example-user:CHANGE-ME -sc show -sc trans --client clientC $@" 
